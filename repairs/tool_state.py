...
class ToolState:
    ...
    def activate(self, target_pos: Optional[Array] = None, 
                target_rot: Optional[Array] = None) -> 'ToolState':
...
def update_tool_state(
    ...
    if action.get('tool_change_slot', -1) >= 0:
        ...
        if tool_state.tool_type == TOOL_NONE or tool_stand.can_change_tool(ee_pos, ee_rot, slot_idx):
            new_tool, success = tool_stand.change_tool(tool_state.tool_type, slot_idx)
            if success:
                tool_state = ToolState(
                    tool_type=new_tool,
                    geometry=tool_stand.get_tool_geometry(new_tool) if new_tool != TOOL_NONE else None
                )
                info['tool_change'] = f"Changed to {TOOL_NAMES.get(new_tool, 'unknown')}"
            else:
    ...
    for obj_id, obj in objects.items():
        ...
        if overlap:
            interactions[obj_id] = {
                ...
                'position': ee_pos,
                'object_pos': obj_pos,
                'timestamp': jp.array(jax.lax.real_of_complex(jp.array(0.0)))  # Placeholder for actual time
            }
