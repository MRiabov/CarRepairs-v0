...
from .geometry.config import (
    ...
    TOOL_STAND_POS, TOOL_STAND_DIMS, TOOL_SLOT_DIMS, TOOL_SLOT_SPACING, Array
)
...
class ToolStand:
    ...
    def _initialize_slots(self) -> List[ToolSlot]:
        ...
        tool_types = [
            TOOL_GRIPPER, TOOL_SUCTION, TOOL_SCREWDRIVER,
            TOOL_WRENCH, TOOL_NONE, TOOL_NONE
        ]
        ...
        for i in range(self.num_slots):
            ...
            pos = TOOL_STAND_POS + jp.array([rel_x, rel_y, TOOL_STAND_DIMS[2]/2 + TOOL_SLOT_DIMS[2]/2])
            
            tool_type = tool_types[i] if i < len(tool_types) else TOOL_NONE
            slots.append(ToolSlot(
                pos=pos,
                tool_type=tool_type,
                occupied=(tool_type != TOOL_NONE)
            ))
    ...
    def find_available_slot(self, tool_type: int = TOOL_NONE) -> Optional[int]:
        ...
        for i, slot in enumerate(self.slots):
            if slot.is_available() and (tool_type == TOOL_NONE or slot.tool_type == tool_type):
                return i
        return None
    ...
    def change_tool(self, current_tool: int, slot_idx: int) -> Tuple[int, bool]:
        ...
        elif current_tool != TOOL_NONE and not slot.occupied and slot.tool_type == TOOL_NONE:
