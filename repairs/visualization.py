...
def create_tool_stand_system(tool_stand) -> System:
    ...
    for i, slot in enumerate(tool_stand.slots):
        ...
        if slot.tool_type != TOOL_NONE:
            ...
            tool_type = (
                "box"
                if hasattr(tool_geom, "size") and len(tool_geom.size) == 3
                else "cylinder"
            )
            size = (
                " ".join(map(str, tool_geom.size))
                if hasattr(tool_geom, "size")
                else "0.05 0.05 0.05"
            )
            rgba = (
                " ".join(map(str, tool_geom.rgba))
                if hasattr(tool_geom, "rgba")
                else "0.5 0.5 0.8 1.0"
            )
    ...
    try:
        ...
        print(
            f"Successfully created MuJoCo model with {mj_model.nq} qpos, {mj_model.nv} qvel"
        )
...
def visualize_tool_stand(
    ...
    try:
        ...
        frames = render_system(
            sys, duration=duration, framerate=framerate, camera=camera
        )
...
def visualize_tool(
    tool_type: int, output_file: Optional[str] = None, open_browser: bool = True
):
    ...
    return visualize_tool_stand(
        tool_stand, output_file=output_file, open_browser=open_browser
    )


def visualize_tool_change(
    tool_stand, tool_state, output_file: Optional[str] = None, open_browser: bool = True
):
    ...
    return visualize_tool_stand(
        viz_stand, output_file=output_file, open_browser=open_browser
    )
